...
def decorate(n, pos, mat):
    """Returned a nxn square decorated matrix where the matrix is placed according
    to lead position"""
    if mat.shape == (n,n):
        return mat
    else:
...
class Green:
    ...
    def do_green_gr(self):
        ...
        sigma_gr = np.matrix(np.zeros((self._n, self._n), dtype =
        np.complex128))
        for key in self._active_leads:
            lead = self._leads[key]
            sigma_gr = sigma_gr +\
            decorate(self._n, lead.get_position(), lead.get_sigma_gr())
    ...
    def do_green_lr(self):
        ...
        for key in self._active_leads:
            lead = self._leads[key]
            sigma_lr = sigma_lr +\
            decorate(self._n, lead.get_position(), lead.get_sigma_lr())
    ...
    def do_eqgreen(self):
        ...
        for key in self._active_leads:
            lead =  self._leads[key]
            es_h = es_h - decorate(self._n, lead.get_position(), lead.get_sigma())
