...
class PhysicalLead(Lead):
    ...
    def __init__(self, name, ham, ham_t, ham_dl, position, over = None, over_t = None,
    over_dl = None, mu = None, temp = 0.0, delta = defaults.delta):
        """A PhysicalLead object describe a semi-infinite periodic lead within the Open Boundary 
        ...
        #These variables are set to none if they have not been calculated, or set to
    ...
    def do_invsurfgreen(self, tol = defaults.surfgreen_tol):
        """Calculate the INVERSE of surface green's function by means of decimation
    ...
    def get_sigma_gr(self):
        ...
        return (1.0 - stats.fermi(self._energy, self._mu, temppot = self._temp)) * (-1j) * self.get_gamma()

    
    def get_sigma_lr(self):
        """Calculate the Sigma lesser"""
        assert(not self._mu is None)
        sigma_lr =  stats.fermi(self._energy, self._mu, temppot = self._temp) * 1j * self.get_gamma()
        return sigma_lr
